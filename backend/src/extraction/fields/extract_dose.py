...
# First we use regex to identify candidate dose phrases in the text
# here are the regex dose patterns 
...
# we extract the candidates below
...
# next we split the candidates into in vivo (mg/kg) vs in vitro (nM/µM) doses based on the units mentioned in the candidate phrases
...
# with the above values, next we get the dose range using min and max from in vivo doses (found above)
...
# finally we call the LLM to validate whether the identified candidates are actual doses, and to extract the route and frequency if mentioned
